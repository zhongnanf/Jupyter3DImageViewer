...
class bokehView3D:
    ...
    def imshow3d(self, img, axis=0, plot_size=1):
        ...
        phys_size_x = size_x*phys_scale
        phys_size_y = size_y*phys_scale
        ...
        sld = ipywidgets.IntSlider(min=0,max=nslices-1,step=1,value=0, layout=layout)
    ...
    def compare_3d_imgs(self, imgs, axis=0, plot_size=1, view_vertical = True):

        nimgs = len(imgs)
        imgs2 = []
        for cnt1 in xrange(nimgs):
        ...
        phys_scale = 10/np.max([size_x, size_y])
        phys_size_x = int(size_x*phys_scale)
        phys_size_y = int(size_y*phys_scale)
        ...
        def slider_value_changed(change):
            for cnt1 in xrange(nimgs):
                renderer = fig_t.select(dict(name='img3d%d'%cnt1, type=GlyphRenderer))
                img_t[cnt1].data_source.data['image'] = [imgs2[cnt1][:,:,sld.value]]
            push_notebook(bk_t)

        sld.observe(slider_value_changed)
        display(sld_tool)

    def compare_aligned_3d(self, img1, img2, axis=0, plot_size=1):
